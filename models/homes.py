...
def insert_home(property_id, agent_id, address, city, state, zip_code, rooms, lot_size, is_current):
    conn = get_conn()
    conn.execute("""
...
def update_home(home_id, property_id, agent_id, address, city, state, zip_code, rooms, lot_size, is_current):
    conn = get_conn()
    conn.execute("""
        UPDATE homes
